...
def _try_extract_typed_text_via_rmscene(document_zip_path: Path, page: int = 1) -> str:
    """
    Best-effort typed-text extraction from reMarkable notebook pages without OCR.

    Strategy:
      - Locate the Nth `.rm` page file inside the downloaded document zip
      - Parse it using `rmscene` (scene_stream/scene_tree/text)
      - Return plaintext if we can, otherwise "".
    ...
    try:
        ...
        rt = importlib.import_module("rmscene.text")

        with zipfile.ZipFile(document_zip_path, "r") as zf:
            rm_members = [n for n in zf.namelist() if n.lower().endswith(".rm")]
            rm_members.sort()
            if not rm_members:
                return ""

            # Page is 1-indexed externally
            idx = max(0, min(len(rm_members) - 1, (page or 1) - 1))
            rm_name = rm_members[idx]
            rm_bytes = zf.read(rm_name)

        # --- Parse scene ----------------------------------------------------
        f = io.BytesIO(rm_bytes)

        scene_or_blocks = None
        for fn_name in ("read_scene", "read", "load", "loads"):
            fn = getattr(ss, fn_name, None)
            if callable(fn):
                try:
                    scene_or_blocks = fn(f)
                    break
                except Exception:
                    f.seek(0)
                    continue

        # Some versions expose read_blocks()
        if scene_or_blocks is None:
            fn = getattr(ss, "read_blocks", None)
            if callable(fn):
                try:
                    f.seek(0)
                    scene_or_blocks = list(fn(f))
                except Exception:
                    scene_or_blocks = None
        ...
        tree = None
        for fn_name in ("build_tree", "scene_to_tree", "to_tree"):
            fn = getattr(st, fn_name, None)
            if callable(fn):
                try:
                    tree = fn(scene_or_blocks)
                    break
                except Exception:
                    continue

        if tree is None:
            # Some versions may already return a tree-like object
            tree = scene_or_blocks

        # --- Extract text ---------------------------------------------------
        TextDocument = getattr(rt, "TextDocument", None)
        doc = None
        if TextDocument is not None:
            # Try common constructors/factory methods
            for meth in ("from_scene_tree", "from_tree", "from_scene"):
                fn = getattr(TextDocument, meth, None)
                if callable(fn):
                    try:
                        doc = fn(tree)
                        break
            ...
            if doc is None:
                try:
                    doc = TextDocument(tree)
                except Exception:
                    doc = None

        # Try to get plaintext from TextDocument
        if doc is not None:
            for attr in ("plain_text", "text", "content"):
                val = getattr(doc, attr, None)
                if isinstance(val, str) and val.strip():
                    return val.strip()
            for meth in ("to_plain_text", "as_text", "__str__"):
                fn = getattr(doc, meth, None)
                if callable(fn):
                    try:
                        s = str(fn()) if meth != "__str__" else str(doc)
                        if s and s.strip():
                            return s.strip()
                    except Exception:
                        continue

        # Fallback: expand text items if we can find them
        expand = getattr(rt, "expand_text_items", None)
        if callable(expand):
            candidates = []
            for attr in ("items", "children", "root"):
                maybe = getattr(tree, attr, None)
                if maybe:
                    candidates.append(maybe)
            try:
                expanded = expand(candidates)
...
async def remarkable_read(
    ...
    """MCP tool: read/extract text from a reMarkable document."""
    return await _remarkable_read_impl(
        document=document,
...
async def _remarkable_read_impl(
    document: str,
    content_type: Literal["text", "raw", "annotations"] = "text",
    page: int = 1,
    grep: Optional[str] = None,
    ...
    try:
        ...
        end_idx = start_idx + page_size        # Handle empty content case (notebooks)
        # We intentionally do NOT auto-run OCR (tesseract) because it produces noisy output for handwritten pages.
        # Instead, we attempt to extract *structured typed text* (e.g., device "Convert to text" output) from .rm
        # files using `rmscene`, which parses reMarkable's scene format.
        if total_chars == 0 and not include_ocr and file_type == "notebook":
            rmscene_text = _try_extract_typed_text_via_rmscene(tmp_path, page=page)
            if rmscene_text:
                content = rmscene_text
                total_chars = len(content)
                result["content"] = content
                result["total_chars"] = total_chars
                result["ocr_backend"] = None
                result["_rmscene_typed_text"] = True
                result["_hint"] = (
                    "Extracted typed text from .rm via rmscene (no OCR). "
                    + result.get("_hint", "")
                )
            ...
            for i, part in enumerate(path_parts):
                ...
                if not found:
                    ...
                    if found_document and i == len(path_parts) - 1:
                        ...
                        read_result = await _remarkable_read_impl(_apply_root_filter(doc_path), page=1)
                        import json
...
async def remarkable_search(
    ...
    try:
        ...
        for doc in documents:
            ...
            try:
                read_result = await _remarkable_read_impl(
                    document=doc["path"],
                    page=1,
                    grep=grep,
                    include_ocr=include_ocr,
