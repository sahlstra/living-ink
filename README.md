...
Unlock the full potential of your reMarkable tablet as a **Living Signal** for AI assistants. This MCP server lets Claude, VS Code Copilot, and other AI tools read, search, and traverse your entire reMarkable library — including handwritten notes via OCR.

<!-- mcp-name: io.github.aaron.livingink -->
...
Copyright (c) 2026 Aaron Sahlstrom

Based on [remarkable-mcp](https://github.com/SamMorrowDrums/remarkable-mcp) by Sam Morrow.
...
## Automation (Background Sync)

To run the sync automatically every hour, we use a macOS `launchd` agent.

**Important:** Due to macOS security restrictions, the automation cannot run directly from `~/Documents`. We use a deployment script to set it up in your Home folder.

1.  **Deploy & Install**:
    ```bash
    ./deploy_automation.sh
    ```
    This creates a copy in `~/remarkable-mcp-automation` and registers the hourly job.
...
3.  **Update**: If you change the code, run `./deploy_automation.sh` again to update the background runner.

See [docs/automation_setup.md](docs/automation_setup.md) for details.
